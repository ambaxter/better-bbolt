...
impl BMByteBadCharShiftMapRev {
  pub fn create_bad_char_shift_map<T: AsRef<[u8]>>(pattern: T) -> Option<BMByteBadCharShiftMapRev> {
    let pattern = pattern.as_ref();
    let pattern_len = pattern.len();
...
pub fn find_full<'a, TT: 'a, TP: 'a>(
  text: &'a TT, pattern: &'a TP, bad_char_shift_map: &BMByteBadCharShiftMap, limit: usize,
) -> Vec<usize>
where
...
pub fn rfind_full<'a, TT: 'a, TP: 'a>(
  text: &'a TT, pattern: &'a TP, bad_char_shift_map: &BMByteBadCharShiftMapRev, limit: usize,
) -> Vec<usize>
where
  TT: Index<usize, Output = u8> + LenTrait + ?Sized,
  &'a TT: IntoIterator<Item = &'a u8>,
...
impl BMByte {
  ...
  pub fn find_first_in<T: AsRef<[u8]>>(&self, text: T) -> Option<usize> {
    find(text.as_ref(), &self.pattern, &self.bad_char_shift_map, 1)
      .first()
      .copied()
  }
