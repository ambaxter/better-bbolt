...
mod test {
  ...
  const U8_CHARSET_SIZE: usize = 256;

  fn bad_character_table(needle: &[u8]) -> [usize; U8_CHARSET_SIZE] {
    let m = needle.len();
    let mut bad_character_table = [m; U8_CHARSET_SIZE];
    let mut i = 0usize;
    while i < m - 1 {
      bad_character_table[needle[i] as usize] = m - i - 1;
      i += 1;
    }
    bad_character_table
  }

  fn suffixes(needle: &[u8]) -> SmallVec<[usize; 16]> {
    let mut f = 0usize;
    let m = needle.len();
    let mut suff = smallvec![0; m];
    suff[m - 1] = m;
    ...
    while i >= 0 {
      if i > g as usize && suff[i + m - 1 - f] < i - g as usize {
        ...
        f = i;
        while g >= 0 && needle[g as usize] == needle[g as usize + m - 1 - f] {
          g -= 1;
        }
        if g < 0 {
          suff[i] = f + g.unsigned_abs();
        } else {
          suff[i] = f - g as usize
        }
      }
  ...
  fn good_character_table(needles: &[u8]) -> SmallVec<[usize; 16]> {
    let m = needles.len();
    let suff = suffixes(needles);
    let mut good_character_table = smallvec![m; m];
    let mut j = 0;
    for i in (0..m).rev() {
      if suff[i] == i + 1 {
        while j < m - 1 - i {
          if good_character_table[j] == m {
            good_character_table[j] = m - 1 - i;
          }
          j += 1;
        }
      }
    ...
    for i in 0..m - 1 {
      good_character_table[m - 1 - suff[i]] = m - 1 - i;
    }
    good_character_table
  }

  #[test]
  fn test_bower() {
    let s = b"test hello xxxxx and hello again test";
    let needle = b"abcde";
    let bad_character_table = bad_character_table(needle.as_slice());
    let good_character_table = good_character_table(needle.as_slice());
    ...
    while j <= n - m {
      ...
      while i >= 0 && needle[i as usize] == s[i as usize + j] {
        i -= 1;
      }
      if i < 0 {
        println!("Matched {}", j);
        j += good_character_table[0];
      } else {
        j += good_character_table[i as usize]
          .max(bad_character_table[s[i as usize + j] as usize] + i as usize + 1 - m);
      }
    }
  }
}
