...
use crate::common::errors::{DiskError, PageError, TxError};
  ...
  IO: IOOverflowPageReader,
{
  ...
  ) -> error_stack::Result<Self::Bytes, DiskError> {
    match self {
  ...
  TX: TxReadLazyPageIO<'tx>,
{
  delegate! {
      to self.tx {
      fn read_freelist_overflow(&self, freelist_page_id: FreelistPageId, overflow: u32) -> crate::Result<<Self::TxPageType as TxPageType<'tx>>::TxPageBytes, DiskError>;
      fn read_node_overflow(&self, node_page_id: NodePageId, overflow: u32) -> crate::Result<<Self::TxPageType as TxPageType<'tx>>::TxPageBytes, DiskError>;
      }
  }
}
